...
import { EightBit } from "../../typechain-types";
...
describe("8bit test", () => {
  ...
  describe("permission check", () => {
    ...
    it("grantMinterRole", async () => {
      ...
      await expect(
        nft.connect(owner).grantMinterRole(user2Addr)
      ).to.emit(nft, "MinterRoleGranted").withArgs(ownerAddr, user2Addr);
    ...
    it("revokeMinterRole", async () => {
      ...
      await expect(
        nft.connect(owner).revokeMinterRole(user1Addr)
      ).to.emit(nft, "MinterRoleRevoked").withArgs(ownerAddr, user1Addr);
  ...
  describe("minting test", () => {
    ...
    it("should success mint nft", async () => {
      ...
      let tokenId = 0;
      await expect(
        nft.connect(owner).safeMint(receiver, tokenId)
      ).to.emit(nft, "Transfer").withArgs(ZERO_ADDRESS, receiver, tokenId);
  ...
  describe("burning test", () => {
    it('should able to burn token by owner', async () => {
      ...
      await expect(
        nft.connect(owner).burn(tokenId)
      ).to.emit(nft, "Transfer").withArgs(receiver, ZERO_ADDRESS, tokenId);

      let tx = nft.connect(owner).ownerOf(tokenId);
    ...
    it('should able to burn token when get approval', async () => {
      ...
      await expect(
        nft.connect(user1).burn(tokenId)
      ).to.emit(nft, "Transfer").withArgs(receiver, ZERO_ADDRESS, tokenId);
  ...
  describe("update base uri test", () => {
    it('has corresponding URI', async () => {
      ...
      await expect(
        nft.connect(owner).updateBaseURI(url)
      ).to.emit(nft, "UpdateBaseUri").withArgs(ownerAddr, url);
    });
  });
